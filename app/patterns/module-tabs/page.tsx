...
export default function ModuleTabs() {
  ...
  const handleMainTabChange = (tabId: string) => {
    setActiveMainTab(tabId);
    // Reset to first sub-tab when changing main tab
    const firstSubTab = subTabs[tabId][0];
    setActiveSubTab(firstSubTab.id);
  };
  ...
  return (
    <div className="module-tabs-container">
      ...
      <div className="main-tabs">
        {mainTabs.map((tab) => (
          <button
            key={tab.id}
            onClick={() => handleMainTabChange(tab.id)}
            className={\`main-tab \${activeMainTab === tab.id ? 'main-tab-active' : ''}\`}
          >
      ...
      <div className="module-tabs">
        {subTabs[activeMainTab].map((subTab) => (
          <button
            key={subTab.id}
            onClick={() => setActiveSubTab(subTab.id)}
            className={\`module-tab \${activeSubTab === subTab.id ? 'module-tab-active' : ''}\`}
          >
            {subTab.label}
          </button>
        ))}
      ...
      <div className="tab-content">
        <h3>Content for {activeMainTab} - {activeSubTab}</h3>
